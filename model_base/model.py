...
class encoder_cada_M(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[500, 500, 1024], z=10):
        super(encoder_cada_M, self).__init__()
...
class decoder_cada_M(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[500, 500, 1024], z=10):
        super(decoder_cada_M, self).__init__()
        self.decoder1 = nn.Sequential(nn.Linear(z,hidden_dim[1]),nn.ReLU(),
                                      nn.Linear(hidden_dim[1],hidden_dim[0]),nn.ReLU(),
                                      nn.Linear(hidden_dim[0],input_dim[0]),nn.Sigmoid())

        self.decoder2 = nn.Sequential(nn.Linear(z,hidden_dim[1]),nn.ReLU(),
                                      nn.Linear(hidden_dim[1],hidden_dim[0]),nn.ReLU(),
                                      nn.Linear(hidden_dim[0],input_dim[1]),nn.Sigmoid())

        self.decoder3 = nn.Sequential(nn.Linear(z,hidden_dim[1]),nn.ReLU(),
                                      nn.Linear(hidden_dim[1],hidden_dim[0]),nn.ReLU(),
                                      nn.Linear(hidden_dim[0],input_dim[2]),nn.Sigmoid())

        self.decoder4 = nn.Sequential(nn.Linear(z,hidden_dim[1]),nn.ReLU(),
                                      nn.Linear(hidden_dim[1],hidden_dim[0]),nn.ReLU(),
                                      nn.Linear(hidden_dim[0],input_dim[3]),nn.Sigmoid())

        self.decoder5 = nn.Sequential(nn.Linear(z,hidden_dim[1]),nn.ReLU(),
                                      nn.Linear(hidden_dim[1],hidden_dim[0]),nn.ReLU(),
...
class encoder_cada_Y(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[2000, 1024, 500], z=10):
        super(encoder_cada_Y, self).__init__()
        self.encoder1 = nn.Sequential(nn.Linear(input_dim[0],hidden_dim[0]),nn.ReLU(),
                                         nn.Linear(hidden_dim[0],hidden_dim[1]),nn.ReLU(),
                                         nn.Linear(hidden_dim[1],hidden_dim[2]),nn.ReLU())
        self.mu1 = nn.Linear(hidden_dim[2], z)
...
class decoder_cada_Y(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[2000, 1024, 500], z=10):
        super(decoder_cada_Y, self).__init__()
        self.decoder1 = nn.Sequential(nn.Linear(z,hidden_dim[1]),nn.ReLU(),
                                      nn.Linear(hidden_dim[1],hidden_dim[0]),nn.ReLU(),
                                      nn.Linear(hidden_dim[0],input_dim[0]),nn.Sigmoid())
...
class encoder_cada_N(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[500, 500, 1024], z=10):
        super(encoder_cada_N, self).__init__()
        self.encoder1 = nn.Sequential(nn.Linear(input_dim[0],hidden_dim[0]),nn.ReLU(),
                                         nn.Linear(hidden_dim[0],hidden_dim[1]),nn.ReLU(),
                                         nn.Linear(hidden_dim[1],hidden_dim[2]),nn.ReLU())
        self.mu1 = nn.Linear(hidden_dim[2], z)
...
class decoder_cada_N(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[500, 500, 1024], z=10):
        super(decoder_cada_N, self).__init__()
        self.decoder1 = nn.Sequential(nn.Linear(z,hidden_dim[1]),nn.ReLU(),
                                      nn.Linear(hidden_dim[1],hidden_dim[0]),nn.ReLU(),
                                      nn.Linear(hidden_dim[0],input_dim[0]),nn.Sigmoid())
...
class encoder_cada_A(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[500, 500, 1024], z=10):
        super(encoder_cada_A, self).__init__()
        self.encoder1 = nn.Sequential(nn.Linear(input_dim[0],hidden_dim[0]),nn.ReLU(),
                                         nn.Linear(hidden_dim[0],hidden_dim[1]),nn.ReLU(),
                                         nn.Linear(hidden_dim[1],hidden_dim[2]),nn.ReLU())
        self.mu1 = nn.Linear(hidden_dim[2], z)
        ...
        self.fuse_weight_1.data.fill_(1 / 4)
        self.fuse_weight_2.data.fill_(1 / 4)
        self.fuse_weight_3.data.fill_(1 / 4)
        self.fuse_weight_4.data.fill_(1 / 4)
    ...
    def forward(self, inputs=[]):
        ...
        z_s = torch.mul(self.fuse_weight_1, z_x1) + torch.mul(self.fuse_weight_2, z_x2) + \
              torch.mul(self.fuse_weight_3, z_x3) + torch.mul(self.fuse_weight_4, z_x4)
        return z_x1, z_x2, z_x3, z_x4,  \
               mu1, mu2, mu3, mu4, logvar1, logvar2, logvar3, logvar4, z_s


class decoder_cada_A(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[500, 500, 1024], z=10):
        super(decoder_cada_A, self).__init__()
    ...
    def forward(self, z_x1, z_x2, z_x3, z_x4, z_s):
        # VAE for feature vector
        recon_x1 = self.decoder1(z_x1)
        recon_x2 = self.decoder2(z_x2)
        recon_x3 = self.decoder3(z_x3)
        ...
        return recon_x1,recon_x2,recon_x3,recon_x4, recon_s1,recon_s2,recon_s3,recon_s4

class encoder_cada_C(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[500, 500, 1024], z=10):
        super(encoder_cada_C, self).__init__()
        self.encoder1 = nn.Sequential(nn.Linear(input_dim[0],hidden_dim[0]),nn.ReLU(),
                                         nn.Linear(hidden_dim[0],hidden_dim[1]),nn.ReLU(),
                                         nn.Linear(hidden_dim[1],hidden_dim[2]),nn.ReLU())
        self.mu1 = nn.Linear(hidden_dim[2], z)
        ...
        self.encoder6 = nn.Sequential(nn.Linear(input_dim[5],hidden_dim[0]),nn.ReLU(),
                                         nn.Linear(hidden_dim[0],hidden_dim[1]),nn.ReLU(),
                                         nn.Linear(hidden_dim[1],hidden_dim[2]),nn.ReLU())
        self.mu6 = nn.Linear(hidden_dim[2], z)
        self.logvar6 = nn.Linear(hidden_dim[2], z)
        ...
        self.fuse_weight_1.data.fill_(1 / 6)
        self.fuse_weight_2.data.fill_(1 / 6)
        self.fuse_weight_3.data.fill_(1 / 6)
        self.fuse_weight_4.data.fill_(1 / 6)
        self.fuse_weight_5.data.fill_(1 / 6)
        self.fuse_weight_6.data.fill_(1 / 6)
    ...
    def forward(self, inputs=[]):
        ...
        x6 = self.encoder6(inputs[5])
        mu6 = self.mu6(x6)
        logvar6 = self.logvar6(x6)
        z_x6 = self.reparameterize(mu6, logvar6)

        z_s = torch.mul(self.fuse_weight_1, z_x1) + torch.mul(self.fuse_weight_2, z_x2) + \
              torch.mul(self.fuse_weight_3, z_x3) + torch.mul(self.fuse_weight_4, z_x4) + \
              torch.mul(self.fuse_weight_5, z_x5) + torch.mul(self.fuse_weight_6, z_x6)

        return z_x1, z_x2, z_x3, z_x4, z_x5, z_x6, mu1, mu2, mu3, mu4, mu5, mu6, \
               logvar1, logvar2, logvar3, logvar4, logvar5, logvar6, z_s


class decoder_cada_C(nn.Module):
    def __init__(self, input_dim=[], hidden_dim=[500, 500, 1024], z=10):
        super(decoder_cada_C, self).__init__()
        self.decoder1 = nn.Sequential(nn.Linear(z,hidden_dim[1]),nn.ReLU(),
                                      nn.Linear(hidden_dim[1],hidden_dim[0]),nn.ReLU(),
                                      nn.Linear(hidden_dim[0],input_dim[0]),nn.Sigmoid())
        ...
        self.decoder6 = nn.Sequential(nn.Linear(z,hidden_dim[1]),nn.ReLU(),
                                      nn.Linear(hidden_dim[1],hidden_dim[0]),nn.ReLU(),
                                      nn.Linear(hidden_dim[0],input_dim[5]),nn.Sigmoid())

    def forward(self, z_x1, z_x2, z_x3, z_x4, z_x5, z_x6, z_s):
        # VAE for feature vector
        recon_x1 = self.decoder1(z_x1)
        recon_x2 = self.decoder2(z_x2)
        recon_x3 = self.decoder3(z_x3)
        ...
        recon_s4 = self.decoder4(z_s)
        recon_s5 = self.decoder5(z_s)
        recon_s6 = self.decoder6(z_s)

        return recon_x1,recon_x2,recon_x3,recon_x4,recon_x5,recon_x6, \
               recon_s1,recon_s2,recon_s3,recon_s4,recon_s5,recon_s6